...
int flash_erase (flash_info_t * info, int s_first, int s_last)
{
	...
	progressbar_init(s_first, s_last+1, 20);
...
/*----------------------------------------------------------------------- 
*/
int flash_get_sect(flash_info_t *info, ulong addr)
{	
    int i;	
    if (info->start[info->sector_count-1] <= addr && addr < CFG_FLASH_BASE + info->size)		
...
int write_buff (flash_info_t * info, uchar * src, ulong addr, ulong cnt)
{
    ...
    if ((addr % CFG_FLASH_BLOCK_SIZE) == 0) {
        if(flash_erase(info, start_sect, end_sect)) { 
            printf("ERROR:flash_erase_1\n");
            return 2;
        }
    } else if (start_sect < end_sect) {
        if(flash_erase(info, start_sect + 1, end_sect)) { 
            printf("ERROR:flash_erase_2\n");
            return 2;
        }
    }
#ifdef CFG_FLASH_USE_BUFFER_WRITE
	...
	if(!(gd->flags & GD_FLG_QUIET))
		progressbar_init(wp, wp+cnt, 20);
	...
	while (cnt >= info->portwidth) {
		...
		wp += info->portwidth;
		cnt -= info->portwidth;
		pg_cnt += info->portwidth;
#endif /* CFG_FLASH_USE_BUFFER_WRITE */
		if(!(pg_cnt & 0xfff) && !(gd->flags & GD_FLG_QUIET))
			 progressbar_show(wp);
	}

	if(!(gd->flags & GD_FLG_QUIET)) {
		progressbar_show(wp+cnt);
		putc('\n');
	}
...
static void flash_write_cmd (flash_info_t * info, flash_sect_t sect, uint offset, uchar cmd)
{
	...
	/* Ensure two writing times' interval is more than 20ns, hisilicon modify */
	udelay(1);
}
...
static int flash_isequal (flash_info_t * info, flash_sect_t sect, uint offset, uchar cmd)
{
	...
	case FLASH_CFI_16BIT:
		...
		/* Disable high 8 bit for 16x8 mode , hisilicon modify */
...
ulong flash_get_size (ulong base, int banknum)
{
	...
	if (flash_detect_cfi (info)) {
		...
		/* buffer_size can allow 256, hisilicon modify */
