...
/*ETH_FrameReceive for one buffer modify by h60016335 in VxWorks*/
#if 0
UINT32 ETH_FrameReceivePacket (Out UINT32 *pu32FrameReceiveDataAddr ,Out UINT32 pu32Length) 
{
	Frame_Receive_Describe ReadDescribEnInf;
	UINT32 CRC_Value = 0,i=0;
	UINT32 * pu32FrameReceiveData=pu32FrameReceiveDataAddr;
	
	  	if (*(UINT32 *) ETH_GLB_REG(11) != 0)
	    {/*receive frame describe available*/
	        
	       for(i=1;i>0;i--)
	       {
	        pu32FrameReceiveData=((UINT32)ETH_CPU_receive);
	        ReadDescribEnInf.w32 = *(UINT32 *) ETH_GLB_REG(10);
			pu32Length=ReadDescribEnInf.bits.fm_len;
		    
	        if(*(UINT32 *)(0xf3f0f010) == 0)/*如果只是收包，则收包后退出*/
			{
			    *(UINT32 *) ETH_GLB_REG(12)=((UINT32)ETH_CPU_receive);/*CPU配置输入队列的存储地址*/
			    break;
	         }
	        ETH_FrameTransmitPacket(pu32Length,pu32FrameReceiveDataAddr);
	             queue_Recv_Tran_uses rtuses;
	             rtuses.w32=(*(UINT32 *) ETH_GLB_REG(16));
	            if(rtuses.bits.tran<32)
	            *(UINT32 *) ETH_GLB_REG(12)=((UINT32)ETH_CPU_receive);/*CPU配置输入队列的存储地址*/
			...
			if (*(UINT32 *)(0xf3f0f004) == 1)
			{
			    CRC_Value =  caculate_crc32(pu32FrameReceiveData, (UINT32)(pu32Length - 4));

			    if (((UINT8)(CRC_Value & 0xff) != *(pu32FrameReceiveData + 
			        pu32Length-1)) ||
			        ((UINT8)((CRC_Value & 0xff00) >> 8) != 
			        *(pu32FrameReceiveData+pu32Length-2))||
			        ((UINT8)((CRC_Value & 0xff0000)>>16) != 
			        *(pu32FrameReceiveData+pu32Length-3))||
			        ((UINT8)((CRC_Value & 0xff000000)>>24) != 
			        *(pu32FrameReceiveData+pu32Length-4)))
			        {
			            printf("CRC Error!\r\n");
			            *(UINT32 *)(0xf3f0f008) += 1;
			        }
                    /*告诉输入帧描述子出队，即收一帧结束*/
		            *(UINT32 *) ETH_GLB_REG(11) = 0;
	       		     /*clear the interrupt status for not enter again*/
...
#endif
#if defined(HI3511_DEMO)||defined(HI3511_UMAP)
